...
import { Shield, Crown, Clock, AlertCircle, Loader2, Info } from "lucide-react";
...
export default function CreateCirclePage() {
  ...
  const { createCircle, isCreating } = useCircleSavings();
  ...
  const [circleForm, setCircleForm] = useState({
    ...
    description: "",
    contribution: "",
    frequency: "1", // 1 is WEEKLY
    maxMembers: "5",
    visibility: "0", // 0 is PRIVATE
  });

  const [step, setStep] = useState(1);

  // Get actual balance from blockchain (in USD)
  const currentBalance = balanceData ? Number(balanceData) / 1e6 : 0;

  const contributionAmount = parseFloat(circleForm.contribution) || 0;
  const maxMembers = parseInt(circleForm.maxMembers);

  // Collateral calculations via hook or local
  const principal = contributionAmount * maxMembers;
  const lateBuffer = principal * 0.01;
  const totalToLock = principal + lateBuffer;
  ...
  const handleCreateCircle = async () => {
    ...
    try {
      const frequencyEnum = parseInt(circleForm.frequency);
      const visibilityEnum = parseInt(circleForm.visibility);
    ...
    >
      ...
      <div className="max-w-4xl mx-auto px-2 sm:px-4 py-4 sm:py-6">
        <div className="space-y-4 sm:space-y-6">
          {/* Step Indicator */}
          <div className="flex items-center justify-between mb-4 sm:mb-8 px-1 sm:px-2 max-w-md mx-auto">
            {[1, 2, 3].map((s) => (
              <React.Fragment key={s}>
                <div className="flex flex-col items-center gap-1 sm:gap-2">
                  <div
                    onClick={() => s < step && setStep(s)}
                    className={`w-8 h-8 sm:w-10 sm:h-10 text-sm sm:text-base rounded-full flex items-center justify-center font-bold transition-all duration-300 ${
                ...
                {s < 3 && (
                  <div
                    className="flex-1 h-1 mx-1.5 sm:mx-4 mb-5 sm:mb-6 rounded-full transition-colors duration-300"
                    style={{
                      backgroundColor:
                        s < step ? colors.primary : colors.border,
                    }}
          ...
          >
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
              <div>
                ...
                >
                  Required: ${totalRequired.toFixed(2)} USDT (collateral +
                  buffer
                  {visibilityFee > 0 ? " + visibility fee" : ""})
                </div>
              </div>
              <div
                className={`w-full sm:w-1/4 px-2 py-1 rounded-full text-[10px] sm:text-sm font-semibold text-center ${
              ...
              >
                ...
                {circleForm.visibility === "1" && (
                  ...
                  >
                    ...
                    $0.50 USDT fee applies for public circles to prevent spam.
                  </div>
                )}
              </div>
          ...
          {step === 2 && (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-4">
                <button
                  onClick={prevStep}
                  className="w-full sm:flex-1 py-3 sm:py-4 text-sm sm:text-base font-bold rounded-xl transition border-2"
                  style={{
                    borderColor: colors.border,
                    color: colors.text,
                    backgroundColor: colors.surface,
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-6 sm:mt-8">
                  ...
                  <button
                    ...
                    className={`w-full sm:flex-2 py-3 sm:py-4 px-2 sm:px-6 text-xs sm:text-base font-bold rounded-xl transition shadow-lg flex items-center justify-center gap-1 sm:gap-2 ${
                      hasSufficientBalance &&
                      circleForm.name &&
                      circleForm.contribution &&
                      !isCreating
