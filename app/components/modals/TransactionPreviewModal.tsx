...
const TransactionPreviewModal: React.FC<TransactionPreviewModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-60 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      >
        {/* Header */}
        <div className="p-5 sm:p-8 flex justify-between items-center border-b border-dashed" style={{ borderColor: colors.border }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[9px] sm:text-[10px] uppercase tracking-widest font-black opacity-40 mt-1" style={{ color: colors.text }}>
                Verification Step
              </p>
            </div>
          </div>
        ...
        <div className="p-5 sm:p-8 space-y-6 sm:space-y-8 overflow-y-auto custom-scrollbar">
          ...
          <div className="space-y-2 sm:space-y-3">
            <p className="text-[10px] sm:text-xs font-black uppercase tracking-widest opacity-40 ml-1" style={{ color: colors.text }}>
              Recipient
            </p>
            {isExternal ? (
              <div
                className="p-4 sm:p-5 rounded-2xl border-2 flex items-center gap-3 sm:gap-4 bg-linear-to-br from-white/50 to-white/10"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="font-black text-xs sm:text-sm break-all leading-tight" style={{ color: colors.text }}>
                    {recipientAddress}
                  </h4>
                  <div className="flex items-center gap-1 mt-1">
                    <div className="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                    <span className="text-[9px] sm:text-[10px] font-bold uppercase tracking-wide opacity-50" style={{ color: colors.text }}>
                      Avalanche Network
                    </span>
                  </div>
                </div>
              ...
              recipientProfile && (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-black text-sm sm:text-base truncate leading-tight" style={{ color: colors.text }}>
                      {recipientProfile.firstName} {recipientProfile.lastName}
                    </h4>
                    <div className="flex items-center gap-1 mt-0.5">
                      <span className="text-xs font-bold truncate max-w-[80px] sm:max-w-none" style={{ color: colors.primary }}>
                        @{recipientProfile.username}
                      </span>
                      <div className="w-0.5 h-0.5 rounded-full bg-gray-300 sm:mx-0.5" />
                      <span className="text-[9px] sm:text-[10px] font-black uppercase opacity-40 whitespace-nowrap" style={{ color: colors.text }}>
                        Internal
                      </span>
                    </div>
                  </div>
          ...
          >
            ...
            <div className="flex justify-between items-center relative z-10 gap-2">
              <span className="text-[10px] sm:text-xs font-bold opacity-50 shrink-0" style={{ color: colors.text }}>
                Transfer Amount
              </span>
              <span className="font-black text-sm sm:text-base truncate" style={{ color: colors.text }}>
                {amount} USDT
              </span>
            </div>
            ...
            <div className="pt-4 border-t border-dashed relative z-10" style={{ borderColor: colors.border }}>
              <div className="flex justify-between items-end gap-2">
                <span className="text-[10px] sm:text-xs font-black uppercase tracking-widest opacity-60 shrink-0 mb-1" style={{ color: colors.text }}>
                  Total
                </span>
                <div className="text-right min-w-0">
                  <div className="flex items-baseline gap-1 justify-end flex-wrap">
                    <h3 className="text-xl sm:text-3xl font-black leading-none break-all" style={{ color: colors.text }}>
                      {total}
                    </h3>
                    <span className="text-[9px] sm:text-[10px] font-black opacity-30">USDT</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          {isExternal && (
            ...
            >
              ...
              <p className="text-[10px] sm:text-[11px] font-bold leading-relaxed text-amber-900/80">
                Ensure the recipient address is correct for the <strong>Avalanche network</strong>. 
        ...
        {/* Footer Actions */}
        <div className="p-5 sm:p-8 border-t" style={{ borderColor: colors.border, backgroundColor: colors.surface }}>
